...
/// <summary>
/// FILE: DigiFikileLms.Application/Features/Content/Commands/ContentCommands.cs
/// LAYER: Application
///
/// WHAT THIS FILE IS:
...
namespace DigiFikileLms.Application.Features.Content.Commands;

/// <summary>
/// TYPE: CreateModuleCommand
/// PURPOSE: An immutable CQRS write request for create module.
...
public record CreateModuleCommand(
    Guid CourseId,
    string Title,
    int OrderIndex) : IRequest<BaseResponse<ModuleDto>>;

/// <summary>
/// TYPE: CreateModuleCommandHandler
/// PURPOSE: The MediatR handler that orchestrates the Create Module Command use case.
/// LMS ROLE: Supports the Content area while respecting the Application layer boundary.
///
/// IMPLEMENTATION GUIDE:
...
/// </summary>
public class CreateModuleCommandHandler : IRequestHandler<CreateModuleCommand, BaseResponse<ModuleDto>>
{
    /// <summary>
    ...
    /// </summary>
    public Task<BaseResponse<ModuleDto>> Handle(CreateModuleCommand request, CancellationToken cancellationToken) =>
        throw new NotImplementedException("TODO: Create module via IModuleRepository.");
}

/// <summary>
/// TYPE: CreateLessonCommand
/// PURPOSE: An immutable CQRS write request for create lesson.
/// LMS ROLE: Supports the Content area while respecting the Application layer boundary.
///
/// IMPLEMENTATION GUIDE:
///   - Keep the record limited to data needed by this write.
...
public record CreateLessonCommand(
    ...
    int DurationMinutes,
    string? ContentUrl,
    string? ContentBody) : IRequest<BaseResponse<LessonDto>>;

/// <summary>
/// TYPE: CreateLessonCommandHandler
/// PURPOSE: The MediatR handler that orchestrates the Create Lesson Command use case.
/// LMS ROLE: Supports the Content area while respecting the Application layer boundary.
///
/// IMPLEMENTATION GUIDE:
///   - Receive already validated request data.
...
///   - Call concrete EF Core, JWT, or hashing types.
/// </summary>
public class CreateLessonCommandHandler : IRequestHandler<CreateLessonCommand, BaseResponse<LessonDto>>
{
    /// <summary>
    /// Executes the handle operation for this type. Pass the supplied CancellationToken to downstream asynchronous work; do not block the thread.
    /// </summary>
    public Task<BaseResponse<LessonDto>> Handle(CreateLessonCommand request, CancellationToken cancellationToken) =>
        throw new NotImplementedException("TODO: Create lesson via ILessonRepository.");
}

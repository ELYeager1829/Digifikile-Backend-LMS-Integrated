...
/// <summary>
/// FILE: DigiFikileLms.Infrastructure/Persistence/Context/DigiFikileLmsDbContextFactory.cs
...
///   Its immediate area is Digi Fikile Lms Db Context Factory; read the individual type comments below before extending it.
///
/// WHY THIS LAYER:
...
/// <summary>
/// TYPE: DigiFikileLmsDbContextFactory
/// PURPOSE: The infrastructure component responsible for digi fikile lms db context factory.
/// LMS ROLE: Supports the Digi Fikile Lms Db Context Factory area while respecting the Infrastructure layer boundary.
///
...
/// </summary>
public class DigiFikileLmsDbContextFactory : IDesignTimeDbContextFactory<DigiFikileLmsDbContext>
{
    /// <summary>
    /// Executes the create db context operation for this type. Keep the operation focused and preserve this layer's dependency boundary.
    /// </summary>
    public DigiFikileLmsDbContext CreateDbContext(string[] args)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            throw new InvalidOperationException(
                "ConnectionStrings__DigiFikileLmsDb is not configured. Set it in .env or the environment.");
        }

        var optionsBuilder = new DbContextOptionsBuilder<DigiFikileLmsDbContext>();
        optionsBuilder.UseNpgsql(
            connectionString,
            options => options.MigrationsAssembly(typeof(DigiFikileLmsDbContext).Assembly.FullName));

        return new DigiFikileLmsDbContext(optionsBuilder.Options);
    }
}
